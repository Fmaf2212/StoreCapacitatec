...
const DetalleProducto = () => {
  ...
  const handleAddToCart = () => {
    // Aquí puedes almacenar el valor de MontoTotal en el localStorage
    localStorage.setItem('MontoTotal', MontoTotal);
    localStorage.setItem('fecha', date);
    localStorage.setItem('idUsuarioLogueado', 14381);
    let valueProducto = [{'idProducto':detalleProducto.IdProduct,'cantidad':cantidad,'precio':detalleProducto.ProductPrice}];
    let jsonProduct = JSON.stringify(valueProducto)
    localStorage.setItem('producto', jsonProduct);
    localStorage.setItem('carritoLength', valueProducto.length);
    alert(`Datos almacenados en el localStorage.`);
  };
